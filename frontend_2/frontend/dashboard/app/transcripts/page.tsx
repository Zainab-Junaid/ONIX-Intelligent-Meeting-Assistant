...
function TranscriptsContent() {
  ...
  const [actionItems, setActionItems] = useState<
    Array<{ id: string; text: string; assignedTo?: string; dueDate?: any }>
  >([]);
  ...
  const { meetings: extensionMeetings, loading: extensionLoading } =
  ...
  useEffect(() => {
    ...
    socket.on(
      "transcript_update",
      (data: { meetingId: string; segments: any[]; timestamp: string }) => {
        if (data.meetingId === meetingId) {
          console.log(
            "📝 Received real-time transcript update:",
          ...
          setSegments((prevSegments) => {
            ...
            data.segments.forEach((seg) => {
              ...
              if (
                !segmentMap.has(key) ||
                segmentMap.get(key).text !== seg.text
              ) {
                segmentMap.set(key, seg);
              }
            });
    ...
    socket.on(
      ...
      (data: { meetingId: string; segments: any[]; timestamp: string }) => {
        if (data.meetingId === botId) {
          console.log(
            "📝 Real-time update for bot meeting:",
            data.segments.length,
            "segments"
          );
          ...
          setLiveSegments((prev) => {
            ...
            prev.forEach((seg) => {
              const key =
                seg.start !== undefined
                  ? `${seg.start}-${seg.speaker}-${seg.text.substring(0, 30)}`
                  : seg.text.substring(0, 50);
              segmentMap.set(key, seg);
            });
            data.segments.forEach((seg) => {
  ...
  if (!authUser)
    return <div className="p-6">Please sign in to view your transcripts.</div>;

  // Show specific extension meeting if extensionId provided
  if (extensionId) {
    const extensionMeeting = extensionMeetings.find(
      (m) => m.id === extensionId
    );
    if (!extensionMeeting)
      return <div className="p-6">Extension meeting not found.</div>;

    return (
      <AppShell
        title={extensionMeeting.title}
        subtitle={`Created ${
          extensionMeeting.createdAt?.toLocaleString("en-US", {
        ...
        <div className="space-y-4">
          ...
          {extensionMeeting.meetingURL && (
            <div className="text-sm text-muted-foreground">
              <a
                href={extensionMeeting.meetingURL}
                target="_blank"
          ...
          <div className="text-xs text-muted-foreground">
            📝 Extension Meeting •{" "}
            {extensionMeeting.autosave ? "Auto-saved" : "Manual save"}
          </div>
        </div>
      </AppShell>
    );
  ...
  if (meetingId) {
    ...
    return (
      <div className="p-6">
        Legacy meeting format not supported. Please use extension or bot
        meetings.
      </div>
    );
  }
  ...
  if (botId) {
    ...
    const displaySegments =
      liveSegments.length > 0 ? liveSegments : botMeeting.segments || [];

    return (
      <AppShell
        title={`${
          botMeeting.title || `Bot Meeting ${botId.substring(0, 8)}...`
        }`}
        subtitle={`Created ${new Date(
          botMeeting.createdAtMs
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {displaySegments.length > 0 && (
                <div className="text-sm text-muted-foreground">
                  ...
                  {displaySegments.length === 1 ? "segment" : "segments"} •{" "}
                  {[...new Set(displaySegments.map((s) => s.speaker))].length}{" "}
                  {[...new Set(displaySegments.map((s) => s.speaker))]
                    .length === 1
                    ? "speaker"
                    : "speakers"}
                </div>
  ...
  return (
    <AppShell
      title="Transcripts"
      subtitle="Auto-captured from meetings Onix joins"
    >
        ...
        <div>
          ...
          <div className="grid gap-3">
            {extensionMeetings.map((meeting) => (
              <a
                key={meeting.id}
                href={`/transcripts?extensionId=${meeting.id}`}
              ...
              >
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  📝 Extension •{" "}
                  {meeting.autosave ? "Auto-saved" : "Manual save"}
                </div>
              </a>
            ))}
            {!extensionLoading && extensionMeetings.length === 0 && (
              <div className="text-sm text-muted-foreground">
                No extension meetings yet. Use the extension to start
          ...
          <div className="grid gap-3">
            {botMeetings.map((meeting) => (
              <a
                ...
                href={`/transcripts?botId=${meeting.meetingId}`}
                className="rounded-lg border p-4 hover:bg-muted/40"
              >
                <div className="flex items-center justify-between">
                  <div className="font-medium">
                    Bot Meeting {meeting.meetingId.substring(0, 8)}...
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {new Date(meeting.createdAtMs).toLocaleString("en-US", {
                      timeZone: "Asia/Karachi",
                      year: "numeric",
                      month: "short",
                      day: "numeric",
...
export default function Page() {
  return (
    <Suspense fallback={<div className="p-6">Loading transcripts...</div>}>
      <TranscriptsContent />
    </Suspense>
  );
}
