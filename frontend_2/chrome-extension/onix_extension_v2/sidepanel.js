...
function initUI() {
  ...
  function updateAuthUI() {
    if (isGuestMode) {
      // Guest mode UI
      authEl.innerHTML = 'Guest Mode <span class="guest-badge">GUEST</span>'
      signInBtn.style.display = ''
      signInBtn.textContent = 'Sign In Instead'
      guestModeBtn.style.display = 'none'
      signOutBtn.style.display = 'none'
      // Disable paste image button in guest mode
      if (pasteImageBtn) {
        pasteImageBtn.style.cursor = 'not-allowed'
        pasteImageBtn.title = 'Not available in guest mode'
      }
      // Update download button text for guest mode
      if (downloadBtn) {
  ...
  signInBtn.addEventListener('click', async () => {
    if (isGuestMode) {
      // If in guest mode, clicking "Sign In Instead" should deactivate guest mode
      await deactivateGuestMode()
      updateAuthUI()
      // Continue to sign in flow immediately
