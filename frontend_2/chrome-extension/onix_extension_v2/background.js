...
// Open side panel automatically on supported hosts (makes it feel attached to the tab)
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if ((isMeet || isZoom) && changeInfo.status === 'complete') {
    ...
    // Auto-open side panel (makes it feel attached to the meeting tab)
...
chrome.tabs.onActivated.addListener(async (activeInfo) => {
  try {
    ...
    if (tab.url) {
      ...
      if (isMeet || isZoom) {
        // Enable and open for meeting tabs
        ...
        // Disable (hide) for non-meeting tabs
        // Setting enabled: false hides the side panel for this specific tab if it was set to specific previously,
        // or just ensures it doesn't show up. 
        // Note: 'enabled: false' removes the side panel icon from the toolbar for this tab, effectively hiding functionality.
        // To strictly 'close' it if it's open globally, we might rely on the fact that side panels are per-tab logic here.
