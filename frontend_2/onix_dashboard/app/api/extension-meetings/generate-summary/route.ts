...
Make the summary thorough, well-organized, and easy to scan. Include specific details, names, and context. Focus on actionable insights and information that will be useful for future reference.`,
      ...
      // Generate action items separately with a more personalized prompt
      actionItemsResp = await client.lemur.task({
        input_text: textToProcess, // Use the translated text
        final_model: "anthropic/claude-sonnet-4-20250514",
        prompt: `Extract every specific action item, task, or assignment discussed in this meeting.
For each item, identify:
1. **item**: A clear, concise description of the task.
2. **assignedTo**: The person responsible. If a specific person is mentioned (e.g., "Mubashra", "John"), use their name. If the speaker says "you" should do something, use "You". If no specific person is mentioned but someone is supposed to do it, use "Unassigned".
3. **dueDate**: Any deadline, date, or time mentioned (e.g., "by Monday", "Nov 5th", "next week"). If no date is mentioned, leave as null.

**Special Instructions:**
- Look for phrases like "Mubashra is supposed to...", "You need to...", "I'll take care of...", "Let's make sure [Name] handles...", "There is a test on [Date]".
- Include general assignments/reminders like "You have a test on Monday" or "Assignment due tomorrow".
- Be thorough - don't miss any tasks.

Return ONLY a JSON array with objects: [{"item": "...", "assignedTo": "...", "dueDate": "..."}]`,
        context: "Personalized action items extraction",
      });
...
- The full transcript is available in the Transcript tab.`,
    ...
    let actionItems: Array<{ text: string; assignedTo?: string; dueDate?: any }> = [];
    try {
      ...
      if (Array.isArray(parsedActionItems)) {
        actionItems = parsedActionItems.map((item: any) => {
          ...
          // Only include assignedTo if it exists and is not undefined/null/empty
          const assignedTo = item.assignedTo || item.assignee || item.assigned_to;
          if (assignedTo && assignedTo.trim() !== '') {
            actionItem.assignedTo = assignedTo.trim();
          }
          // Only include dueDate if it exists and is valid
          if (item.dueDate) {
            try {
          ...
          return actionItem;
        }).filter(item => item.text && item.text.trim() !== ''); // Remove any items without text
      }
    } catch (parseError) {
