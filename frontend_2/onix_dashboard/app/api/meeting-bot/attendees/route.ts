...
export async function POST(request: NextRequest) {
  try {
    ...
    const refreshToken = userData?.googleRefreshToken || userData?.refreshToken; // Some legacy fields might vary
    ...
    if (!calendarEventId && meetingUrl) {
        ...
        try {
            ...
            if (meetCodeMatch) {
                 ...
                 const now = new Date();
                 // Look back 7 days and ahead 1 day, covering potential past meetings
                 ...
                 if (matchedEvent) {
                     ...
                     // Update meeting doc with matched event
