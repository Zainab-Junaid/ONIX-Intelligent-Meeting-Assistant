...
export async function GET(request: NextRequest) {
  try {
    ...
    const dbUrl =
      ...
      "postgresql://meetingbot:supersecret@localhost:5432/meetingbotpoc";
    ...
    try {
      ...
      userMeetingIds = jobResult.rows
        .map((r: any) => r.meetingId)
        .filter(Boolean);
    } catch (dbError: any) {
      console.warn(
        "Could not query MeetingJob for userId filtering:",
        dbError.message,
      );
    } finally {
    ...
    if (!res.ok) {
      return NextResponse.json(
        {
          error: "Failed to fetch summaries from backend",
