...
interface AskOnixSheetProps {
  ...
  meetingId: string;
  meetingTitle: string;
  /** Optional: segments already loaded in the UI. When provided, these are sent to the API so Q&A works even if backend lookup fails. */
  segments?: Segment[];
}

export function AskOnixSheet({ open, onOpenChange, meetingId, meetingTitle, segments: propSegments }: AskOnixSheetProps) {
  ...
  // Reset welcome message when meeting changes
  useEffect(() => {
    setMessages([
      {
        id: 'welcome',
  ...
  const handleSend = async () => {
    ...
    try {
      let segmentsToSend = Array.isArray(propSegments) && propSegments.length > 0 ? propSegments : [];
      if (segmentsToSend.length === 0 && meetingId && authUser) {
        try {
          ...
          // Ignore fetch error, proceed with empty segments
        }
      }
